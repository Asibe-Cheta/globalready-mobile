...
const rawCountryDialCodes: CountryDialCode[] = [
  ...
  { name: 'Congo, The Democratic Republic of the', dialCode: '+243' },
...
const normalizedMap = rawCountryDialCodes.reduce<Record<string, CountryDialCode>>(
  (acc, entry) => {
    const normalized = normalizeCountryName(entry.name);
    if (!acc[normalized]) {
...
Object.entries(aliasMap).forEach(([alias, canonical]) => {
  const target = normalizedMap[normalizeCountryName(canonical)];
  if (target) {
    normalizedMap[normalizeCountryName(alias)] = target;
  }
});

export const resolveCountryName = (value: string) => {
  const normalized = normalizeCountryName(value);
  const entry = normalizedMap[normalized];
  return entry?.name || null;
};

export const getCountryDialCode = (value?: string) => {
  if (!value) return null;
  const normalized = normalizeCountryName(value);
  const entry = normalizedMap[normalized];
  return entry?.dialCode || null;
};

export const isValidCountryName = (value: string) => {
  const normalized = normalizeCountryName(value);
  return Boolean(normalizedMap[normalized]);
};

export const countryNames = Object.values(normalizedMap)
