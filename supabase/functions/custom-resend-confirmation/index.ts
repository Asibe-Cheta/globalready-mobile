...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, fullName } = (await req.json()) as Payload;
    ...
    // Generate a new confirmation link for the existing user
    ...
    if (sendgridApiKey && fromEmail) {
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;">
                    <tr>
                      <td style="padding:32px 24px 12px;">
                        ...
                        <h2 style="margin:16px 0 8px;font-size:22px;color:#0f172a;">Confirm your email</h2>
                        <p style="color:#475569;font-size:14px;line-height:1.6;">
                          Hi ${fullName?.trim() || 'there'}, tap the button below to confirm your email and access your GlobalReady account.
                        </p>
                      </td>
                    </tr>
                    <tr>
      ...
      if (!sgResponse.ok) {
        ...
        console.error('SendGrid resend error:', details);
        return new Response(
